...
class DineInRepository implements DineInRepositoryInterface {
  ...
  Future<DineInModel?> getRestaurantList({int? offset, required bool isDistance, required bool isRating, required bool isVeg, required bool isNonVeg, required bool isDiscounted, required List<int> selectedCuisines}) async {
    ...
    String cuisine = Uri.encodeComponent(jsonEncode(selectedCuisines));
    Response response = await apiClient.getData('${AppConstants.dineInRestaurantListUri}?offset=${offset ?? 1}&limit=10&sort_by=${isDistance ? 'distance' : isRating ? 'rating' : ''}&veg=${isVeg ? 1 : 0}&non_veg=${isNonVeg ? 1 : 0}&discount=${isDiscounted ? 1 : 0}&cuisine=$cuisine');
