...
class HomeRepository implements HomeRepositoryInterface {
  ...
  @override
  Future<CategoryCuisineModel?> getCategoryCuisineList({String? type, String? name, int? limit, int? offset, DataSourceEnum? source}) async {
    CategoryCuisineModel? categoryCuisineModel;
    String uri = '${AppConstants.categoryCuisineUri}?type=${type ?? 'all'}&name=${name ?? ''}&limit=${limit ?? 10}&offset=${offset ?? 1}';

    switch(source!) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(uri);
        if(response.statusCode == 200) {
          categoryCuisineModel = CategoryCuisineModel.fromJson(response.body);
      ...
      case DataSourceEnum.local:
        ...
        if(cacheResponseData != null) {
          categoryCuisineModel = CategoryCuisineModel.fromJson(jsonDecode(cacheResponseData));
        }
    }

    return categoryCuisineModel;
  }
