...
class _SignInViewState extends State<SignInView> {
  ...
  Widget activeCentralizeLogin(CentralizeLoginSetup centralizeLoginSetup, AuthController authController) {
    ...
    print('centralizeLogin: $centralizeLogin');
  ...
  void _otpLogin(AuthController authController, String countryDialCode, CentralizeLoginType loginType) async {
    ...
    if(_formKeyLogin!.currentState!.validate()) {
      ...
      } else if(authController.otpResendRemainingSeconds(countryDialCode + phone) > 0) {
        _navigateToOngoingOtpVerification(authController, countryDialCode + phone);
      } else if(authController.otpResendRemainingSeconds(numberWithCountryCode) > 0) {
        _navigateToOngoingOtpVerification(authController, numberWithCountryCode);
      } else {
  ...
  void _navigateToOngoingOtpVerification(AuthController authController, String number) {
    if(ResponsiveHelper.isDesktop(Get.context)) {
      Get.back();
      Get.dialog(VerificationScreen(
        number: number, email: null, token: '', fromSignUp: true, fromForgetPassword: false,
        loginType: CentralizeLoginType.otp.name, password: '', firebaseSession: authController.lastOtpSession,
      ));
    } else {
      Get.toNamed(RouteHelper.getVerificationRoute(
        number, null, '', RouteHelper.signUp, null, CentralizeLoginType.otp.name,
        session: authController.lastOtpSession,
      ));
    }
  }
