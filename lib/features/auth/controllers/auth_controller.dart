...
class AuthController extends GetxController implements GetxService {
  ...
  String? _lastOtpSession;
  String? get lastOtpSession => _lastOtpSession;

  void recordOtpSent(String number, {String? session}) {
    _lastOtpNumber = number;
    _lastOtpSentAt = DateTime.now();
    _lastOtpSession = session;
  }

  int otpResendRemainingSeconds(String? number) {
    if(number == null || _lastOtpNumber != number || _lastOtpSentAt == null) return 0;
    int elapsed = DateTime.now().difference(_lastOtpSentAt!).inSeconds;
    return (otpCooldownSeconds - elapsed).clamp(0, otpCooldownSeconds);
  }
  ...
  Future<ResponseModel> otpLogin({required String phone, required String loginType, required String otp, required String verified, bool alreadyInApp = false}) async {
    ...
    _getUserAndCartData(responseModel);
    if(responseModel.isSuccess && otp.isEmpty && responseModel.authResponseModel != null && !responseModel.authResponseModel!.isPhoneVerified!
  ...
  Future<void> firebaseVerifyPhoneNumber(String phoneNumber, String? token, String loginType, {bool fromSignUp = true, bool canRoute = true, UpdateUserModel? updateUserModel})async {
    ...
    Timer(const Duration(seconds: 30), () {
      if(!settled) {
        stopLoading();
        showCustomSnackBar('otp_send_failed_please_try_again'.tr);
      }
    });
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      codeSent: (String vId, int? resendToken) {
        ...
        stopLoading();
        recordOtpSent(phoneNumber, session: vId);
