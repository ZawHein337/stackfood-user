...
class SearchService implements SearchServiceInterface {
  ...
  Future<List<String>?> getTrendingSearches() async {
    return await searchRepositoryInterface.getTrendingSearches();
  }

  @override
  Future<TopCategoryModel?> getTopCategories({int offset = 1, int limit = 10}) async {
    return await searchRepositoryInterface.getTopCategories(offset: offset, limit: limit);
  }

  @override
  Future<RestaurantModel?> getFeaturedRestaurants({int offset = 1, int limit = 10}) async {
    return await searchRepositoryInterface.getFeaturedRestaurants(offset: offset, limit: limit);
  }

  @override
  Future<RestaurantModel?> getExclusiveDeals({int offset = 1, int limit = 10}) async {
    return await searchRepositoryInterface.getExclusiveDeals(offset: offset, limit: limit);
  }
  ...
  Future<Response> getSearchData({required String query, required bool isRestaurant, required int offset,
    ...
    type: type, isNew: isNew, freeDelivery : freeDelivery, isAvailableFood: isAvailableFood, isPopular: isPopular, isOneRatting: isOneRatting, isTwoRatting: isTwoRatting, isThreeRatting: isThreeRatting,
        ...
        minPrice: minPrice, maxPrice: maxPrice, selectedCuisines: selectedCuisines, orderType: orderType, isOpenRestaurant: isOpenRestaurant, nearMe: nearMe);
  }
