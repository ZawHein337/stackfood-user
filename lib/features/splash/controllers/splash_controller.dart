...
class SplashController extends GetxController implements GetxService {
  ...
  Future<void> getConfigData({bool handleMaintenanceMode = false, DataSourceEnum source = DataSourceEnum.local, NotificationBodyModel? notificationBody, bool fromMainFunction = false, bool fromDemoReset = false}) async {
    ...
    if(source == DataSourceEnum.local) {
      ...
      getConfigData(handleMaintenanceMode: handleMaintenanceMode, source: DataSourceEnum.client, notificationBody: notificationBody);
    } else {
  ...
  void _handleConfigResponse(Response response, bool handleMaintenanceMode, bool fromMainFunction, bool fromDemoReset, {required NotificationBodyModel? notificationBody, required DeepLinkBody? linkBody}) {
    if(response.statusCode == 200) {
      ...
      if(_configModel != null) {
        ...
        } else if(kDebugMode && !Get.currentRoute.startsWith(RouteHelper.splash)) {
          debugPrint('[nav] splash route() SKIPPED - already left splash. currentRoute=${Get.currentRoute} source=${response.statusCode}');
        } else if(Get.currentRoute.startsWith(RouteHelper.splash)) {
          debugPrint('[nav] splash route() RUNNING. currentRoute=${Get.currentRoute} notificationBody=${notificationBody?.notificationType}');
