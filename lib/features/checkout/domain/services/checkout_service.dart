...
class CheckoutService implements CheckoutServiceInterface {
  ...
  @override
  Future<DeliveryCoverageModel?> getDeliveryCoverageList({int? restaurantId}) async {
    return await checkoutRepositoryInterface.getDeliveryCoverageList(restaurantId: restaurantId);
  }
  ...
  @override
  Future<DistanceDurationModel> getDistanceAndDuration(LatLng originLatLng, LatLng destinationLatLng) async {
    double distance = -1;
    int? duration;
    ...
    try {
      if (response.statusCode == 200) {
        duration = parseDurationInSecond(response.body['duration']);
    ...
    return DistanceDurationModel(distanceInKm: distance, durationInSecond: duration, isFallback: isFallback);
  }

  int? parseDurationInSecond(dynamic duration) {
    if(duration == null) {
      return null;
    }
    final double? seconds = double.tryParse(duration.toString().replaceAll(RegExp(r'[^0-9.]'), ''));
  ...
  @override
  Future<Response> getCheckoutSummary(CheckoutSummaryBodyModel summaryBody, {String? guestId}) async {
    return await checkoutRepositoryInterface.getCheckoutSummary(summaryBody, guestId: guestId);
  }
