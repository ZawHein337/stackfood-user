...
class ReviewRepository implements ReviewRepositoryInterface {
  ...
  @override
  Future<List<Product>?> getList({int? offset, VegType? type, DataSourceEnum? source}) async {
    List<Product>? reviewedProductList;
    String cacheId = AppConstants.reviewedProductUri;

    switch(source!){
      case DataSourceEnum.client:
        Response response = await apiClient.getData('${AppConstants.reviewedProductUri}?type=${(type ?? VegType.all).value}');
