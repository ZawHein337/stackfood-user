  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
        ...
        if(response.payload!.isNotEmpty) {
          ...
          if(payload.notificationType == NotificationType.order) {
            if(Get.find<AuthController>().isGuestLoggedIn()){
              Get.to(()=> const DashboardScreen(pageIndex: 1, fromSplash: false));
            }else {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if(message.data['type'] == AppConstants.demoResetTopic) {
        showCustomDialog(
          child: const DemoResetDialogWidget(),
          isDismissible: false,
        );
      ...
      if(message.data['type'] == 'message' && Get.currentRoute.startsWith(RouteHelper.messages)) {
        ...
        NotificationHelper.showNotification(message, flutterLocalNotificationsPlugin);
        if(Get.find<AuthController>().isLoggedIn()) {
          Get.find<OrderController>().getMyOrders(MyOrderTabType.running, 1);
          Get.find<OrderController>().getMyOrders(MyOrderTabType.history, 1);
