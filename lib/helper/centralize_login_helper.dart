...
class CentralizeLoginHelper {
  static bool isSocialLoginActive(ConfigModel? configModel, String medium) {
    CentralizeLoginSetup? setup = configModel?.centralizeLoginSetup;
    if(setup?.socialLoginStatus != true) {
      return false;
    }
    bool enabledInPanel = medium == 'google' ? (setup?.googleLoginStatus ?? false)
        : medium == 'facebook' ? (setup?.facebookLoginStatus ?? false)
        : false;
    return enabledInPanel && isSocialMediumActive(configModel?.socialLogin, medium);
  }

  static bool isSocialMediumActive(List<SocialLogin>? socialLogin, String medium) {
    return socialLogin?.any((login) =>
        login.loginMedium?.toLowerCase().trim() == medium && (login.status ?? false)) ?? false;
  }
