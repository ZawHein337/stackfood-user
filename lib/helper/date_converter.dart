...
class DateConverter {
  ...
  static String? dineInDateTime(String? dateTime) {
    ...
    return DateFormat('MMM d, yyyy ${_timeFormatter()}').format(parsed);
  }
  ...
  static String timeRange(DateTime start, DateTime end) {
    final bool is24 = Get.find<SplashController>().configModel!.timeformat == '24';
    if (is24) {
      final DateFormat f = DateFormat('HH:mm');
      return '${f.format(start)} - ${f.format(end)}';
    }
    final DateFormat time = DateFormat('h:mm');
    final DateFormat period = DateFormat('a');
    final bool samePeriod = period.format(start) == period.format(end);
    return samePeriod
        ? '${time.format(start)} - ${time.format(end)} ${period.format(end)}'
        : '${time.format(start)} ${period.format(start)} - ${time.format(end)} ${period.format(end)}';
  }
  ...
  static String convertMinutesToDayHourMinute(int minutes) {
    ...
    return '${days != 0 ? '${days}d' : ''} ${hours != 0 ? '${hours}hr' : ''} ${remainingMinutes != 0 ? '${remainingMinutes}min' : ''}';
  }
